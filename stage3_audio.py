...
STAGE_3_GOOGLE_SYSTEM_PROMPT = """You are an expert Voiceover Director. Convert the provided array of English sentences into an expressive script ready for Google TTS.
...
def validate_google_tts_lines(lines: List[str], original_sentences: List[str]):
...
def call_google_tts_api(text_content: str, voice_name: str, output_filepath: Path):
...
def generate_stage3_audio(
    ...
    """
    الدالة المحورية للمرحلة الثالثة:
    1. التحقق من الصوت (رجالي فقط).
    2. صياغة النص التعبيري عبر Gemini.
    3. التحقق الصارم من القواعد (انفعالات، إيموجي، أنماط).
    4. استدعاء API المحرك المختار لإنتاج ملف صوتي مدمج.
    """
    ...
    if engine == "azure":
        ...
        if voice not in GOOGLE_MALE_VOICES:
            raise ValueError(f"الصوت '{voice}' غير مسموح به! الأصوات المعتمدة حصراً لـ Google هي رجالية: {GOOGLE_MALE_VOICES}")

        logger.info(f"🎙️ تجهيز مسار Google Cloud TTS بالصوت الرجالي: {voice}")
        user_prompt = f"Convert these {len(sentences)} sentences with emotions and emojis:\n" + "\n".join(f"- {s}" for s in sentences)

        google_output = call_gemini_with_fallback(
            system_instruction=STAGE_3_GOOGLE_SYSTEM_PROMPT,
            user_prompt=user_prompt,
            response_mime_type="text/plain"
        )
        
        google_cleaned = re.sub(r"^```(?:text)?\s*", "", google_output.strip())
        google_cleaned = re.sub(r"\s*```$", "", google_cleaned).strip()
        lines = [l.strip() for l in google_cleaned.split("\n") if l.strip()]

        validate_google_tts_lines(lines, sentences)
        call_google_tts_api("\n".join(lines), voice, final_audio_file)

    else:
        raise ValueError(f"محرك الصوت '{engine}' غير مدعوم! المحركات المتاحة: 'azure' أو 'google'")
